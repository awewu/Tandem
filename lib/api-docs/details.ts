...
const OVERRIDES: Record<string, Partial<ApiEndpointDoc>> = {
  '/api/auth/login': {
    ...
    statuses: [
      ...
      { code: '403', description: '账号已被禁用。' },
      { code: '409', description: '账号已创建但尚未初始化密码，需要管理员重置密码。' },
      { code: '423', description: '账号因连续登录失败被临时锁定。' },
  ...
  '/api/integrations/plm/daily-reports': {
    summary: 'Innovation Studio 同步当前登录人的结构化日报到 Tandem。',
    requestHeaders: [
      field('Authorization', 'Bearer <access_token>', 'POST 必填。Tandem OIDC access token，必须包含 api.write scope；作者与租户只从 token 解析。', true),
      field('Content-Type', JSON_TYPE, 'POST 必填。请求体编码格式。', true),
      field('Idempotency-Key', 'string', '推荐与 externalReportId 相同；实际幂等键为 tenantId + authorId + sourceSystem + externalReportId。'),
      field('Cookie', 'tandem_at=<access-token>', 'GET 可用，用于 Tandem 日报页读取当前用户的结构化日报。'),
      field('x-request-id', 'string', '可选。调用方传入后服务端会透传；未传时由 middleware 自动生成。'),
    ],
    bodyParams: [
      field('schemaVersion', '"plm.daily-report.v1"', '契约版本。', true),
      field('sourceSystem', '"innovation-studio"', '来源系统。', true),
      field('externalReportId', 'string', 'PLM 侧日报幂等 ID。不同用户可相同，服务端会叠加 token 中的 tenantId/authorId。', true),
      field('reportDate', 'string', '日报日期，YYYY-MM-DD。', true),
      field('replaceExisting', 'boolean', 'true 表示本次 entries 是该日报完整数据，服务端会删除已移除条目对应的接口生成 check-in。', true),
      field('entries[].externalEntryId', 'string', 'PLM 侧条目幂等 ID，同一日报内必须唯一。', true),
      field('entries[].krId', 'string | null', '关联 KR ID。null 表示非 OKR 工作，仅保存到 DailyReport。', true),
      field('entries[].projectCode', 'string', '项目或工作分类编码。', true),
      field('entries[].hours', 'number', '结构化工时数值，非负。', true),
      field('entries[].workType', 'string', '工作类型。', true),
      field('entries[].content', 'string', '工作成果内容；不会拼接日期或工时。', true),
    ],
    responseFields: [
      field('ok', 'boolean', '是否同步成功。'),
      field('reportId', 'string', 'Tandem DailyReport ID。'),
      field('authorId', 'string', 'token sub 对应的用户 ID。'),
      field('reportDate', 'string', '日报日期。'),
      field('entryCount', 'number', '当前 DailyReport 条目数。'),
      field('updated', 'boolean', '是否更新了既有日报。'),
      field('dailyReports', 'DailyReport[]', 'GET 返回当前登录人可见的结构化日报。'),
    ],
    statuses: [
      { code: '200', description: '同步或查询成功。' },
      { code: '400', description: '请求体、日期、entries 或 KR 状态校验失败。' },
      { code: '401', description: '未提供有效登录态，或 POST 未使用 OIDC Bearer access token。' },
      { code: '403', description: 'Bearer token 缺少 api.write scope，或当前用户不是 KR owner/co-owner。' },
      { code: '409', description: '既有 DailyReport 关联的 check-in 不属于本接口生成记录。' },
      { code: '500', description: '服务端异常。响应体通常包含 error 字段。' },
    ],
  },
  '/api/oidc/token': {
