...
export async function buildCompanyBrainSystemPrompt(opts?: {
  ...
  const lines = [
    ...
    '',
    '【回答方式 · 先自由推理, 再对齐】',
    '- 请针对问题做出你判断下最好的分析: 有推理链、可下结论、给方向与优先级, 按问题复杂度自由展开 (简单问题简短, 复杂问题充分)。',
    '- 上面的公司 Memory / OKR 是你作答的重要依据与背景, 请自然融入; 如相关可点出"这服务哪个 OKR", 但不必生硬套用固定句式。',
    '- 语气分析型, 不情绪化。(答案交付前会经出口对齐镜片按公司基线/OKR 复核, 你只需专注给出最好的分析。)',
    '',
    okrContext,
    '',
