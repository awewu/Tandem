...
export default function AdminInvitePage() {
  ...
  return (
    <main className="container mx-auto max-w-3xl space-y-4 p-6">
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <p className="mb-1 text-xs text-muted-foreground">绑定邮箱 (可选)</p>
              <input
                type="email"
                value={email}
            ...
            <div>
              <p className="mb-1 text-xs text-muted-foreground">预设角色 (逗号分隔)</p>
              <input
                type="text"
                value={roles}
            ...
            <div>
              <p className="mb-1 text-xs text-muted-foreground">有效期 (小时)</p>
              <input
                type="number"
                value={validHours}
            ...
            <div>
              <p className="mb-1 text-xs text-muted-foreground">最大使用次数</p>
              <input
                type="number"
                value={maxUses}
          ...
          {lastCode && (
            <div className="rounded border border-emerald-300 bg-emerald-50 p-3">
              ...
              <div className="flex items-center gap-2 rounded bg-white p-2">
                <code className="flex-1 text-lg font-bold tracking-widest">{lastCode}</code>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">已发出的邀请</CardTitle>
        </CardHeader>
        <CardContent>
          {list.length === 0 ? (
            <p className="text-sm text-muted-foreground">尚无邀请记录</p>
          ) : (
