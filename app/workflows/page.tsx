  ...
  return (
    <div className="min-h-screen bg-surface-1 px-4 py-4 text-ink-primary md:px-6">
      <main className="mx-auto max-w-[1540px] space-y-4">
        {showWorkflowHeader && (
        <section className="rounded-md border border-border bg-surface-0 shadow-sm">
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <Button type="button" onClick={startNewModel} className="bg-[#e60039] text-white hover:bg-[#c90032]">
            ...
            {groups.map((group) => (
              <button
                ...
                className={`flex h-12 w-full items-center justify-between rounded-md border px-3 text-left text-body font-medium ${
                  groupFilter === group.name ? 'border-[#ff9aa8] bg-[#fff1f3] text-ink-primary' : 'border-transparent hover:bg-surface-1'
                }`}
              >
...
function WorkflowModelEditor(props: {
  ...
  return (
    <form className="overflow-hidden rounded-md border border-border bg-surface-0 shadow-sm" onSubmit={(e) => void props.onSubmit(e)}>
  ...
  return (
    <div className="grid min-h-[520px] overflow-hidden rounded-md border border-border bg-surface-0 xl:grid-cols-[220px_minmax(0,1fr)_340px]">
      ...
      <section className="min-w-0 overflow-hidden border-r border-border">
        ...
        <div className="overflow-hidden bg-[#f6f8fb] p-4">
              ...
              {props.formDraft.fields.map((field, index) => (
                <div
                  ...
                  className={`grid w-full min-w-0 cursor-pointer items-center gap-3 overflow-hidden border bg-surface-0 p-3 text-left lg:grid-cols-[112px_minmax(0,1fr)_116px] ${
                    selectedFieldIndex === index && propertyTab === 'field' ? 'border-brand-500 shadow-sm' : 'border-dashed border-[#dbe7f5] hover:border-brand-300'
                  }`}
                >
  ...
  return (
    <div className="overflow-hidden rounded-md border border-border bg-surface-0">
      ...
      <div className="min-h-[520px]">
        <section className="relative min-h-[520px] overflow-auto bg-[#f3f5f8] p-6">
          <div className="absolute left-6 top-6 z-10 inline-flex items-center rounded-md border border-border bg-surface-0 shadow-sm">
            <button
              type="button"
                  ...
                  <div key={`${node.id}-${index}`} className="flex w-full flex-col items-center">
                    {isEnd ? (
                      ...
                      <button
                        type="button"
                        onClick={() => setSelectedIndex(index)}
                        className={`w-[240px] overflow-hidden rounded-sm bg-surface-0 text-left shadow-md transition ${selected ? 'ring-2 ring-brand-500' : 'hover:ring-1 hover:ring-brand-500'}`}
                      >
                    ...
                    {index < nodes.length - 1 && (
                      <div className="flex flex-col items-center">
                        <div className="h-5 w-px bg-[#d8e0eb]" />
                        <button
                          type="button"
