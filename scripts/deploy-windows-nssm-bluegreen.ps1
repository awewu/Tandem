...
$PackageName = Normalize-Arg $PackageName
...
function Reload-Nginx {
  $nginxArgs = @()
  if (-not [string]::IsNullOrWhiteSpace($NginxPrefix)) {
    $nginxArgs += @("-p", $NginxPrefix)
  }
  if (-not [string]::IsNullOrWhiteSpace($NginxConfig)) {
    $nginxArgs += @("-c", $NginxConfig)
  }

  & $NginxExe @nginxArgs -t
  if ($LASTEXITCODE -ne 0) {
    throw "nginx config test failed with exit code $LASTEXITCODE"
  }

  & $NginxExe @nginxArgs -s reload
  if ($LASTEXITCODE -ne 0) {
