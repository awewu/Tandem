...
### 6. 用户要求上传代码时默认传完整源码改动

当用户说"上传代码"、"都给我上传"、"传完整"、"其他修改也上传"时, 默认含义是: 当前工作区内所有源码/文档改动都要提交并推送到指定分支, 不能只按刚完成的功能范围选择部分文件.

**强制要求**:
- 上传前必须执行 `git status --short`, 把已修改、已删除、未跟踪文件完整看一遍.
- 必须区分"源码/文档改动"与"临时产物/截图/构建包/浏览器快照"; 源码和文档默认要上传.
- 排除任何文件前, 必须能说清楚原因. 常见可排除项: `.playwright-cli/`、截图输出、APK、构建产物、临时调试 JSON.
- 如果用户说"完整"或"都上传", 不得只提交当前任务相关文件; 除非用户明确要求只传某个模块或某个文件清单.
- 推送前必须用 `git diff --cached --name-only` 复核 staged 清单, 确认没有漏掉源码文件, 也没有混入临时产物.
- 最终回复必须说明: 推送的 commit、包含的主要文件范围、仍未上传的文件及原因.
...
```
1. 确认目标 remote / branch, 避免推错仓库
2. 执行 git status --short, 完整盘点所有变更
3. 分类: 源码/文档默认上传; 临时产物/截图/构建包默认排除并说明
4. 如果用户要求"完整/都上传", 不得只传当前任务相关文件
5. stage 后执行 git diff --cached --name-only 复核清单
6. 执行 npx tsc --noEmit 和 git diff --cached --check
7. commit 并 push
8. 最终回复列出 commit、推送目标、未上传项及排除原因
```
