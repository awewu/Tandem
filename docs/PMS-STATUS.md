...
| D2 | ~~**83 处 `any`**~~ ✅ **已彻底清零 (83 → 0)** | 2026-07-24 分 5 批: **`lib/pms` 全目录 `any` 计数 = 0** (`grep any lib/pms` 无结果)。① 6 处 `input: any` → 内联输入接口 (rebate/product-catalog/quote-rec/alert create)。② 全部 list/get 读路径返回 → `ReturnType<typeof mapXxx>` (提取/复用命名 mapper)。③ create/transition ad-hoc 返回 → 移除 `Promise<any>` 注解让 TS 从 return 精确推断。④ `Record<string, any>` patch → `Partial<typeof pmsX.$inferInsert>`。⑤ jsonb `as any` 写入 → 直接赋值 (列无 `$type`, 插入类型为 unknown, 无需 cast)。⑥ `catch (err: any)` → `catch (err)` + 显式类型守卫。**关键判断**: `lib/types/pms.ts` 与实际 schema 字段名分叉 (dealerLevel↔productLine 等), 故以 drizzle schema (`$inferSelect/$inferInsert`) 为数据 SSOT, 用贴合实际用法的内联接口。附带修正 alert `targetRole` 与 schema notNull 不符。 |
