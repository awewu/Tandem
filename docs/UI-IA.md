...
|---|---|---|
| 🏠 首页 | 🏠 首页 | 不变 |
| 📊 事半 (含 OKR/议事/IM/Memory) | 📊 事半 + 💬 沟通 + 📚 知识 + 📋 流程 | 事半项太多, 拆出沟通(IM+议事)/知识(文档+Memory)/流程(审批+日程) 减压 |
| ✨ 拿捏 | ✨ 拿捏 | 不变 |
| — | ✉️ **邮箱** (新加) | PRD v1 时无此功能; 邮箱有独立心智 (收/发/抄送/外发), 不归任何母模块 |
| — | 📢 内网 (新加) | 公司动态门户; v1 误把它当首页段 2, 实际深度页流量足以独立 |
| — | 🏢 组织 (新加) | 三省六部制项目机制可视化; PRD 没规定但保留 |
| 🛠️ 管理 | 🛠️ 管理 | 含 /admin/intranet 等编辑入口 |
| ⚙️ 设置 | ⚙️ 设置 | 含 /settings/appearance (新加: 主题+Logo 切换) |

### 0.2 收益 / 代价

**收益**:

- 每个模块 SubSidebar items 数量降到 5 个内, 不再 17 项一锅粥
- 邮箱独立保留外部协同的正式感, 与 IM 决议型已读清晰区分
- 内网独立后, 公告/政策/CEO 周记可深度 (V1 蓝本下只是首页一段, 容不下年鉴/社交/Town Hall)

**代价**:

- AppRail 槽位从 5 升到 10, 触及 Teams 8 个的舒适上限
- 部分用户初次进入需多扫一眼定位

**抑制再扩张**: 顶级模块 V2 锁定为 10. 后续新功能进二级, 不再加 AppRail 槽位.

### 0.3 邮箱独立的具体理由

| 维度 | 邮箱 | IM | 议事室 |
|---|---|---|---|
| 主用户 | 个人 (对外) | 团队 (内部) | 跨职能 (决议) |
| 收件人 | 一个或几个, 显式 | 频道全员 | 受邀参与者 |
| 留痕诉求 | 强 (法律证据) | 中 (决议型已读) | 强 (决议卡 + Memory) |
| 节奏 | 异步, 小时-天 | 准实时 | 17 分钟收敛 |
| 心智模型 | 邮件 inbox | 频道 channel | 议程 + 投票 |

不重叠, 各占独立模块.

---

## 1. 顶级导航 (左侧 sidebar · 5 大区) — v1 原始蓝本
