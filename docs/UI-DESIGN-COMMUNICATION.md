...
所有表格、业务列表、审批列表、异常列表、导入预览和错误明细列表, 只要数据来自接口或可能超过一屏, 都必须分页展示.

**交互要求**:
- 列表底部必须显示总条数、当前页 / 总页数、每页条数.
- 至少提供上一页、下一页; 数据量大的主列表可增加页码、每页条数切换或快速跳页.
- 批量操作默认只作用于当前页已选项, 文案必须体现"本页"或明确影响范围.
- 空状态、加载中、切页中、无更多数据都需要有稳定状态, 切页不能导致布局跳动.
- 禁止把导入错误、撞单明细、审批待办、冲突列表等高数据量内容一次性平铺到页面.
