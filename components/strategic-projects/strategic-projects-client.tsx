  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="mt-1 h-10 w-full rounded-lg border border-border bg-[rgb(var(--surface-1))] px-3 text-sm text-ink-primary outline-none focus:ring-2 focus:ring-[rgb(var(--brand-200))]"
      />
    </label>
  );
}
  ...
  return (
    <label className="block">
      ...
      <textarea
        ...
        defaultValue={defaultValue}
        rows={rows}
        className="mt-1 w-full resize-y rounded-lg border border-border bg-[rgb(var(--surface-1))] px-3 py-2 text-sm leading-relaxed text-ink-primary outline-none focus:ring-2 focus:ring-[rgb(var(--brand-200))]"
      />
    </label>
  );
}
  ...
  return (
    <div ref={pickerRef} className="relative md:col-span-2">
      ...
      <button
        ...
        className="mt-1 flex h-10 w-full items-center justify-between gap-2 rounded-lg border border-border bg-[rgb(var(--surface-1))] px-3 text-left text-sm text-ink-primary outline-none focus:ring-2 focus:ring-[rgb(var(--brand-200))]"
      >
      ...
      {open && (
        <div className="absolute z-30 mt-1 w-full rounded-lg border border-border bg-[rgb(var(--surface-1))] p-2 shadow-soft-lg">
          <input
            ...
            autoFocus
            className="mb-2 h-9 w-full rounded-lg border border-border bg-[rgb(var(--surface-1))] px-3 text-sm text-ink-primary outline-none focus:ring-2 focus:ring-[rgb(var(--brand-200))]"
          />
...
function FormActions({ saving, onCancel, formId }: { saving: boolean; onCancel: () => void; formId: string }) {
  return (
    <>
      <button type="button" onClick={onCancel} className="h-9 rounded-lg border border-border px-4 text-sm text-ink-secondary">
        取消
      </button>
      <button type="submit" form={formId} disabled={saving} className="inline-flex h-9 items-center gap-2 rounded-lg bg-[rgb(var(--brand-500))] px-4 text-sm font-semibold text-white disabled:opacity-60">
      ...
      footer={
        <>
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={saving}
            className="h-9 rounded-lg bg-danger px-4 text-sm font-semibold text-white disabled:opacity-60"
          >
            确认删除
          </button>
        </>
  ...
  return (
    <main className="min-h-full bg-[rgb(var(--surface-2))] text-ink-primary">
      <div className="flex gap-3 px-3 py-4">
        <aside className="hidden w-[150px] shrink-0 rounded-xl bg-[rgb(var(--surface-1))] p-3 shadow-soft-sm md:block">
          ...
          <div className="px-6 py-4">
            ...
            <div className="overflow-hidden rounded-xl border border-border">
              <div className="overflow-x-auto">
                <table className="w-full min-w-[1508px] table-fixed border-collapse text-sm">
                  <colgroup>
                          ...
                          <div className="group/menu absolute right-3 top-1/2 z-20 -translate-y-1/2">
                            ...
                            <div className="absolute left-8 top-1/2 hidden w-24 -translate-y-1/2 rounded-lg border border-border bg-[rgb(var(--surface-1))] p-1 shadow-soft-lg group-hover/menu:block group-focus-within/menu:block">
                              <button
                                ...
                                className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm text-ink-secondary hover:bg-[rgb(var(--surface-2))] hover:text-danger"
                              >
      ...
      >
        ...
        <label className="block md:col-span-2">
          ...
          <textarea
            ...
            className="mt-1 min-h-24 w-full rounded-lg border border-border bg-[rgb(var(--surface-1))] px-3 py-2 text-sm text-ink-primary outline-none focus:ring-2 focus:ring-[rgb(var(--brand-200))]"
          />
        </label>
      </form>
    </FixedModal>
  ...
  if (!project) {
    return (
      <main className="min-h-full bg-[rgb(var(--surface-2))] p-6 text-ink-primary">
        ...
        <div className="mt-6 rounded-xl bg-[rgb(var(--surface-1))] p-8 text-ink-secondary">项目不存在或已被删除。</div>
      </main>
    );
  }
    ...
    <main className="min-h-full bg-[rgb(var(--surface-2))] text-ink-primary">
      ...
      {detailViewMode === 'card' ? (
        <section ref={milestoneScrollerRef} className="snap-x snap-mandatory overflow-x-auto scroll-smooth px-6 py-4">
          <div className="flex min-w-max gap-3">
            {project.milestones.map((milestone) => {
              ...
              >
                ...
                <div className="mt-3 min-h-[calc(100vh-278px)] rounded-t-xl bg-[rgb(var(--brand-50))]/55 p-4">
                  <button onClick={() => setTaskModal({ milestoneId: milestone.id })} className="mb-3 flex h-12 w-full items-center justify-center rounded-lg bg-[rgb(var(--surface-1))] text-sm font-medium text-[rgb(var(--brand-600))]">
                    <Plus className="h-4 w-4" />
                    创建任务
                  </button>
            ...
            <div
              ...
              className="w-[340px] shrink-0 snap-start scroll-ml-6"
            >
              <button onClick={() => setMilestoneModal(null)} className="flex h-[74px] w-full items-center justify-center rounded-r-xl bg-[rgb(var(--surface-3))] px-4 text-sm font-medium text-ink-tertiary">
                <Plus className="h-4 w-4" />
                创建里程碑
              </button>
            </div>
        ...
        <section className="px-6 py-4">
          <div className="overflow-x-auto rounded-xl border border-border bg-[rgb(var(--surface-1))]">
      ...
      {showTaskHelp && (
        <FixedModal
          ...
          footer={
            <button type="button" onClick={() => setShowTaskHelp(false)} className="h-9 rounded-lg bg-[rgb(var(--brand-500))] px-4 text-sm font-semibold text-white">
              知道了
            </button>
          }
        >
